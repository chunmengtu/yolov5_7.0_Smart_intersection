...
def run(
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], det[:, 4])):
                        ...
                        #车辆计数
                        # Check if the car ID has been detected before
                        if id not in detected_car_ids:
                            # If not, add it to the set and increment the counter
                            detected_car_ids.add(id)
                            car_counter += 1
            ...
            # 显示车辆计数
            cv2.putText(im0, f'Number of vehicles: {car_counter}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 215, 0),
                        6)
            # Stream results
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=Path, default='yolov5/runs/train/exp/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--reid-weights', type=Path, default=WEIGHTS / 'osnet_x0_25_msmt17.pt')
    parser.add_argument('--tracking-method', type=str, default='strongsort', help='strongsort, ocsort, bytetrack')
    parser.add_argument('--source', type=str, default='yolov5/data/video/cut.mkv', help='file/dir/URL/glob, 0 for webcam')
