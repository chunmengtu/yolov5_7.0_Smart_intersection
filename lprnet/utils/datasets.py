...
###################################################################
# 制作mask
image_label = cv2.imread('ces/label.png')  # 根据自己路径修改mask位置
...
class LoadImages:  # for demo
    ...
    def __next__(self):
        ...
        # 画线
        cv2.line(img0, (0, 1099), (2007, 901), (0, 255, 255), 2)
        ...
        # 绘制文字信息
        draw.text((1700, 930), "车牌识别区域", font=font, fill=(0, 255, 255))
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        s = np.stack([letterbox(x, new_shape=self.img_size)[0].shape for x in self.imgs], 0)  # demo shapes
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect demo if all shapes equal
        if not self.rect:
